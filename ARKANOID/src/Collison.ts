...
export class Collision {
  ...
  // Checar a colisão da bola com o "Brick"
  ...
  checkBallCollision(ball: Ball, paddle: Paddle, view: CanvasView): void {
    // 1. Checar a colisão da bola com o "Paddle"
    if (
    ...
    // 2. Checar a colisão da bola com a parede
    // Movimento da Bola com o eixo X
    if (ball.pos.x > view.canvas.width - ball.width || ball.pos.x < 0) {
      ball.changeXDirection();
    }
    // Movimento da Bola com o eixo Y
    if (ball.pos.y < 0) {
      ball.changeYDirection();
    }
  }
